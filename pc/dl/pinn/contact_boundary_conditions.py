...
from dl.pinn.geometry import Single_Cube_Geometry


class Contact_Boundary_Condition:
    """Encapsulates contact boundary data for traction or pressure application.
    
    This object holds:
      * `geom`: the contact surface geometry (face or patch).
      * `pressure`: a uniform normal pressure applied to that surface.
    
    Downstream methods can compute boundary tractions, tangential components,
    gap functions, complementary conditions, etc., using this data.
    
    Attributes:
        geom (Single_Cube_Geometry): Geometry of the contact patch.
        pressure (torch.Tensor): Scalar normal pressure on `geom.device`.
    """
    def __init__(self, geom: Single_Cube_Geometry, pressure: float):
        """Initialize a contact boundary condition.
        
        Converts the scalar `pressure` to a tensor on the same device as `geom`.
        
        Args:
            geom (Single_Cube_Geometry): Geometry helper describing the contact patch.
            pressure (float): Scalar pressure (positive in compression).
        """
        self.geom = geom
        self.pressure = torch.tensor(pressure, device=geom.device)
    
    def calculate_traction_from_stress(self, sigma_xx: torch.Tensor, sigma_yy: torch.Tensor, sigma_zz: torch.Tensor,
                sigma_xy: torch.Tensor, sigma_yz: torch.Tensor, sigma_zx: torch.Tensor,
                normals: torch.Tensor, t1: torch.Tensor, t2: torch.Tensor):
        ...
        Args:
            sigma_xx: Tensor of shape (N,) for sig_xx.
            sigma_yy: Tensor of shape (N,) for sig_yy.
            sigma_zz: Tensor of shape (N,) for sig_zz.
            sigma_xy: Tensor of shape (N,) for sig_xy = sig_yx.
            sigma_yz: Tensor of shape (N,) for sig_yz = sig_zy.
            sigma_zx: Tensor of shape (N,) for sig_zx = sig_xz.
    ...
    def calculate_traction_mixed_formulation(self, inputs: torch.Tensor, outputs: torch.Tensor
        ...
        Args:
            ...
            outputs: FloatTensor of shape (M,9), the network's predictions in
                the order `[u, v, w, sig_xx, sig_yy, sig_zz, sig_xy, sig_yz, sig_zx]`.
    ...
    def calculate_tangential_traction_component1(self, inputs: torch.Tensor, outputs: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            ...
            outputs: FloatTensor of shape (M,9), network predictions in order
                    [u, v, w, sig_xx, sig_yy, sig_zz, sig_xy, sig_yz, sig_zx].
