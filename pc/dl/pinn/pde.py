...
class PDE:
    ...
    Attributes:
        net (nn.Module):
            ...
            displacements and stress components [u, v, w, sxx, syy, szz, sxy, syz, szx].
        E (float):
    ...
    def calculate_stress_tensor(self, inputs: torch.Tensor
        ...
        Returns:
            ...
            - sigma_xx: normal stress sxx
            - sigma_yy: normal stress syy
            - sigma_zz: normal stress szz
            - sigma_xy: shear stress sxy = syx
            - sigma_yz: shear stress syz = szy
            - sigma_zx: shear stress szx = sxz
        """
    ...
    def calculate_stress_coupling(self, inputs: torch.Tensor, outputs: torch.Tensor
        ...
        displacement field.  This method returns the six component-wise differences:
            ...
            term_xx = sxx(ε) - sxx^net,
            term_yy = syy(ε) - syy^net,
            …,
            term_zx = szx(ε) - szx^net.
        
        Args:
            inputs:  FloatTensor of shape (N,3), the spatial coordinates.
            outputs: FloatTensor of shape (N,9), network predictions in the order
                    [u, v, w, sxx^net, syy^net, szz^net, sxy^net, syz^net, szx^net].
                    
        Returns:
            A 6-tuple of FloatTensors, each of shape (N,):
            - term_xx: Residual in sxx.
            - term_yy: Residual in syy.
            - term_zz: Residual in szz.
            - term_xy: Residual in sxy.
            - term_yz: Residual in syz.
            - term_zx: Residual in szx.
        """
    ...
    def pde_mixed(self, inputs: torch.Tensor, outputs: torch.Tensor
        ...
        Returns:
            ...
            - momentum_x: ∂sxx/∂x + ∂sxy/∂y + ∂sxz/∂z
            - momentum_y: ∂sxy/∂x + ∂syy/∂y + ∂syz/∂z
            - momentum_z: ∂sxz/∂x + ∂syz/∂y + ∂szz/∂z
        ...
        def stress_fn(x):
            # returns (6,) vector [sxx, syy, szz, sxy, syz, szx] for a single point
