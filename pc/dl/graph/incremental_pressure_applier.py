...
class Incremental_Pressure_Applier_Graph:
    ...
    This class drives a loop of:
        ...
        2. predicting top‑face response (displacement, stress, strain),
        3. updating the damage state and degraded modulus,
        4. updating the geometry for the next increment.
        
    Attributes:
        pinn (PINN_Graph): Configured PINN solver with geometry, PDE, and BCs.
        dmg (Damage): Damage model that computes stiffness degradation.
    """
    def __init__(self, pinn: PINN_Graph, dmg: Damage) -> None:
        """Initialize with a PINN instance and a damage model.
        
        Args:
            pinn (PINN_Graph): The PINN solver to train and predict displacements and stresses.
            dmg (Damage): The damage model used to degrade material stiffness.
        """
        self.pinn = pinn
        self.dmg  = dmg
    ...
    def apply_load(self, pressure: float, n_step: int, num_epochs: int) -> None:
        ...
        print("Saved load-disp history to log/graph/load_disp.csv")
    ...
    def save_to_vtu(self, n_step: int) -> None:
        ...
        gmsh.write(f'log/graph/paraview/mesh.msh')
        gmsh.finalize()
        # 2) Read mesh with meshio
        m = meshio.read(f'log/graph/paraview/mesh.msh')
    ...
    def plot_load_disp(self) -> Tuple[plt.Figure, plt.Axes]:
        ...
        This method loads experimental, FEM, Telichko, PINN, and PINN_Graph
