...
from fem.parameters import Damage_Parameter
from fem.fem.materials import Material
from fem.fem.geo_mesh import Mesh
from fem.utils.log_config import setup_logger
from fem.utils.utils import plot_template, macaulay_bracket, calculate_equivalent_strain_from_voigt, calculate_principal_values
...
class Mazars_Original_Damage(Damage):
    ...
    def __init__(self, mat: Material, para: Damage_Parameter) -> None:
        """Initializes the Mazars damage model using material properties and damage evolution parameters.
...
class Mazars_Original_Damage_Torch(Damage):
    ...
    def calculate_stress_tensor(self, eps: torch.Tensor, d: torch.Tensor) -> torch.Tensor:
        ...
        For each of N points, we have a small-strain tensor ε (3x3) and a scalar
    ...
    def calculate_damage(self, eps: torch.Tensor, d0: torch.Tensor, clip: bool = True) -> torch.Tensor:
        ...
        in Voigt notation and return Nx3 damage states:
...
class Mu_Damage(Damage):
    ...
    def calculate_r(self, d0: float, D: np.ndarray, eps: np.ndarray) -> float:
        ...
        Args:
            ...
            D (np.ndarray): Constitutive stiffness matrix (6x6).
