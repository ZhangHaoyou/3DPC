...
class Incremental_Pressure_Applier_Graph:
    ...
    def apply_load(self, pressure: float, n_step: int, num_epochs: int) -> None:
        ...
        Args:
            pressure:  Pressure increment to apply at each step.
            n_step:    Number of loading increments.
            num_epochs: Number of Adam+L-BFGS epochs per increment.
    ...
    def predict(self, x: torch.Tensor, edge_index: torch.Tensor, n_step: int) -> torch.Tensor:
        ...
        Args:
            x:       FloatTensor of shape (N,3) physical coords.
            n_step:  Number of incremental load steps applied.
